...
import time
import random
import donkeycar.parts.sonicrangesensor
import donkeycar.parts.hc_sr04
...
'''传感器模型'''
import pigpio
from donkeycar.parts.sonicrangesensor import Sensor
'''传感器模型'''
from donkeycar.parts import sonicrangesensor
from donkeycar.parts import hc_sr04
def drive(cfg, model_path=None, use_joystick=False, model_type=None, camera_type='single', meta=[] ):
    ...
    # from donkey.part.sonicrangesensor import Sensor  # 传感器类
    range = Sensor(pi, cfg.RANGE_GPIOS)
    # diss=range.run_threaded()
    # print(diss)
    ...
    class MySwitch:
        ...
        def __init__(self):
            ...
            # pi = pigpio.pi()
            # RANGE_GPIOS = [5, 6]
            # self.range = Sensor(pi, RANGE_GPIOS)
        ...
        def run(self):
            ...
            #if (self.pilot_state):
               ## time.sleep(5)
            # ultrasonic hr-sr04 code

            #dis = self.range.run_threaded()  # 距离信息什么时候加载出来?
            dis = range.run_threaded()
            #print(dis)
            # time.sleep(5)
            if(dis<=100):
                return False
    ...
    class MyPilot:
        ...
        def update(self):
            while True:
                self.angle, self.throttle = self.run_threaded()
                print(self.angle)

        def run(self):
            return (0,0)
            #返回的tuple第一个值是angle，第二个值是throttle
            #此模版随机返回向左转或向右转，throttle一直为零

    V.add(MyPilot(), inputs=[], outputs=['mypilot/angle','mypilot/throttle'])

    #Choose what inputs should change the car.
    class DriveMode:
        #在DriveMode里新增根据run_mypilot的判断
        def run(self, mode,
                    user_angle, user_throttle,
                    pilot_angle, pilot_throttle,
                    mypilot_angle, mypilot_throttle, run_mypilot):
            # if run_mypilot:
    ...
    V.add(DriveMode(),
          inputs=['user/mode', 'user/angle', 'user/throttle', 'pilot/angle', 'pilot/throttle',
                  'mypilot/angle', 'mypilot/throttle', 'run_mypilot'],
